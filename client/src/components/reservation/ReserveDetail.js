...
const ReserveDetail = ({ match, history }) => {
  const { index, id } = match.params;
  const items = [
    { to: '/', span: '홈' },
    { to: '/reservation/check-reserve', span: '예약조회' },
    { to: `/reservation/check-reserve/${id}/${index}`, span: '회원 상세예약 조회' },
  ];
  const { userReserve } = useSelector(({ checkReserve }) => ({
    userReserve: checkReserve.userReserve[index],
  }));
  ...
  const onClick = () => {
    history.push('/reservation/check-reserve');
  };

  const { booking, roomType, detail, bookingID, TotalPrice } = userReserve;
